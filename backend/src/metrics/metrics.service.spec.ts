...
describe('MetricsService TCP metrics', () => {
  it('exports bounded TCP connection, frame, rejection, and duration metrics', async () => {
    ...
    metrics.tcpCommandDuration.observe({ protocol: 'wavve', function_code: '0x03e9', result: 'succeeded' }, 0.02);
    ...
    expect(output).toContain('anees_tcp_command_duration_seconds_count{protocol="wavve",function_code="0x03e9",result="succeeded"} 1');
  });
});
